...
/*
 ...
 Возможные ошибки:
- в качестве аргумента метода в массив строк может быть передан null - 
  в итоге получим NullPointerException при попытке получения размерности; 

- в цикле по j используется "магическое" число, если размерность массива окажется меньше -
  получим выход за пределы индекса ArrayIndexOutOfBoundsException;

- элементы массива могут содержать не только числовые значения,
 что вызовет NumberFormatException при попытке получить int val.
